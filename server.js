...
function start(c){const r=rooms[c];if(!r||Object.keys(r.players).length<2)return;r.phase="countdown";r.count=3;r.winner=null;Object.values(r.players).forEach(p=>{p.progress=0;p.roundScore=0;p.catches=0;p.x=130+Math.random()*740});emit(c);
 let n=3;const cd=setInterval(()=>{if(!rooms[c])return clearInterval(cd);r.count=n--;emit(c);if(n<0){clearInterval(cd);r.phase="playing";r.time=r.round===2?35:25;emit(c);timer(c)}},700)}
function finish(c,id){const r=rooms[c];if(!r||r.phase!=="playing")return;const a=Object.values(r.players).sort((x,y)=>y.progress-x.progress);a.forEach((p,i)=>{p.roundScore=Math.round(p.progress)+[120,80,55,40][i];p.total+=p.roundScore});r.winner=id||a[0]?.id;r.phase="result";emit(c);setTimeout(()=>{if(!rooms[c])return;if(r.round<2){r.round++;r.phase="intermission";r.winner=null}else r.phase="score";emit(c)},3200)}
io.on("connection",s=>{
 s.on("join",({code,name})=>{code=clean(code,"BABUY26");rooms[code]??=mk();const r=rooms[code];if(Object.keys(r.players).length>=4)return s.emit("full");s.join(code);s.data.code=code;r.host??=s.id;const used=new Set(Object.values(r.players).map(p=>p.avatar));let avatar=[0,1,2,3].find(i=>!used.has(i))??0;r.players[s.id]={id:s.id,name:clean(name),avatar,total:0,roundScore:0,progress:0,catches:0,x:150};s.emit("me",s.id);emit(code)});
 ...
 s.on("move",({dx})=>{const r=rooms[s.data.code],p=r?.players[s.id];if(!p||r.phase!=="playing"||r.round!==2)return;p.x=Math.max(55,Math.min(945,p.x+(Number(dx)||0)));emit(s.data.code)});
 s.on("catch",({hit})=>{const r=rooms[s.data.code],p=r?.players[s.id];if(!p||r.phase!=="playing"||r.round!==2||!hit)return;p.catches++;p.progress=Math.min(100,p.progress+20);if(p.progress>=100)finish(s.data.code,s.id);else emit(s.data.code)});
 ...
 s.on("again",()=>{const r=rooms[s.data.code];if(!r||r.host!==s.id)return;r.phase="lobby";r.round=0;r.winner=null;Object.values(r.players).forEach(p=>{p.total=0;p.progress=0;p.catches=0});emit(s.data.code)});
...
});server.listen(process.env.PORT||8080,()=>console.log("ARDELL 26 V8 cartoon rebuild"));
