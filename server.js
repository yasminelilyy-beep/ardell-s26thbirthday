...
function stopChalk(c){const h=chalkTimers.get(c);if(h){clearInterval(h);chalkTimers.delete(c)}}
...
function chalkLoop(c){const r=rooms[c];stopChalk(c);r.chalk=[];let tick=0;const handle=setInterval(()=>{const rr=rooms[c];if(!rr||rr.phase!=='playing'||rr.round!==2){stopChalk(c);return}tick++;if(tick%6===0&&rr.chalk.length<14){rr.chalk.push({id:++rr.chalkSeq,x:7+Math.random()*86,y:-8-Math.random()*9,s:1.0+Math.random()*.45})}
 ...
 if(Object.values(rr.players).some(p=>p.progress>=100)){const w=Object.values(rr.players).sort((a,b)=>b.progress-a.progress)[0];stopChalk(c);return finish(c,w?.id)}
 if(tick%2===0)emit(c);
},50);chalkTimers.set(c,handle)}
...
function finish(c,id){const r=rooms[c];if(!r||r.phase!=='playing')return;stopChalk(c);const a=Object.values(r.players).sort((x,y)=>y.progress-x.progress);a.forEach((p,i)=>{p.roundScore=Math.round(p.progress)+[120,80,55,40][i];p.total+=p.roundScore});r.winner=id||a[0]?.id;r.phase='result';emit(c);setTimeout(()=>{if(!rooms[c])return;if(r.round<2){r.round++;r.phase='intermission';r.winner=null;r.chalk=[]}else r.phase='score';emit(c)},2800)}
io.on('connection',s=>{
 ...
 s.on('again',()=>{const r=rooms[s.data.code];if(!r||r.host!==s.id)return;stopChalk(s.data.code);r.phase='lobby';r.round=0;r.winner=null;r.chalk=[];Object.values(r.players).forEach(p=>{p.total=0;p.progress=0;p.catches=0});emit(s.data.code)});
 s.on('disconnect',()=>{const r=rooms[s.data.code];if(!r)return;delete r.players[s.id];if(r.host===s.id)r.host=Object.keys(r.players)[0]||null;if(!Object.keys(r.players).length){stopChalk(s.data.code);delete rooms[s.data.code]}else emit(s.data.code)})
});
server.listen(process.env.PORT||8080,()=>console.log('ARDELL 26 V10.2 chalk stream fix'));
