...
function chalkLoop(c){const r=rooms[c];stopChalk(r);r.chalk=[];let tick=0;r.chalkTimer=setInterval(()=>{const rr=rooms[c];if(!rr||rr.phase!=='playing'||rr.round!==2){stopChalk(rr);return}tick++;if(tick%3===0&&rr.chalk.length<14){rr.chalk.push({id:++rr.chalkSeq,x:7+Math.random()*86,y:-8-Math.random()*9,s:1.0+Math.random()*.45})}
 rr.chalk.forEach(ch=>ch.y+=ch.s*2.1);
...
io.on('connection',s=>{
 ...
 s.on('move',dir=>{const r=rooms[s.data.code],p=r?.players[s.id];if(!p||r.phase!=='playing'||r.round!==2)return;dir=Math.sign(Number(dir)||0);p.x=Math.max(5,Math.min(95,p.x+dir*4.8));emit(s.data.code)});
